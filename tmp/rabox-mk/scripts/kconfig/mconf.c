...
"Overview\n"
"--------\n"
"Some features may be built directly into netsvc.\n"
...
"between different netsvc configurations.\n"
"\n"
...
"------------------------------\n"
"If you prefer to have all of the netsvc options listed in a single\n"
...
load_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep several different netsvc\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than \n"
	"netsvc's default, entering the name of the file here will allow you\n"
...
save_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep different netsvc\n"
	"configurations available on a single machine.\n"
	"\n"
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("NetSvc %s Configuration"),
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new netsvc configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the netsvc configuration.\n"
				"Your netsvc configuration changes were NOT saved."
				"\n\n"));
			return 1;
		}
		printf(_("\n\n"
			"*** End of netsvc configuration.\n"
			"*** Execute 'make' to build netsvc or try 'make help'."
			"\n\n"));
	} else {
		fprintf(stderr, _("\n\n"
			"Your netsvc configuration changes were NOT saved."
